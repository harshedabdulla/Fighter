...
const updateGame = (p1,p2,gameState) => {
  ...
  if (p1.health<=0||p2.health<=0) {
    let winner = p1.health>p2.health?p1.name:p2.name
    resultDiv.innerHTML=`<p>Game over. Winner: ${winner}</p>`
  }
}
...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    updateGame(player,enemy,"strike")
...
class Game {
  ...
  checkIsOver(p1, p2) {
    this.isOver = p1.health <= 0 || p2.health <= 0;
  }
  ...
  play(p1, p2) {
    ...
    while (!this.isOver) {
      ...
      p1.strike(p2,randomNumber(1,10))
      p1.heal(randomNumber(1,5))
      p2.strike(p1,randomNumber(1,10))
      p2.heal(randomNumber(1,5))
    }
...
const player1 = new Player('Harshed', 100, 7);
const player2 = new Player('Atul', 100, 7);
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'Q' && enemyHealth > 0 && isOver === false) {
    strike();
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'a' && playerHealth > 0 && isOver === false) {
    heal();
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'p' && enemyHealth > 0 && isOver === false) {
    strike();
    // After striking then play attack sound
    playAttackSound();
  }
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'l' && playerHealth > 0 && isOver === false) {
    heal();
    // After healing then play heal sound
    playHealSound();
  }
